...
function strToAESBitArray(str, key) {
    ...
    for(var i = 0;i < bits.length;i += 4) {
        var thisSlice = sjcl.bitArray.bitSlice(bits,32*i,32*i + 128);
        toReturn = sjcl.bitArray.concat(toReturn,a.encrypt(thisSlice));
    }
...
function AESBitArrToString(cphr, key) {
    console.log("Entered AESBitArrToString.")
    var a = new sjcl.cipher.aes(key);
    var padded_stuff = [];
    for(var i = 0;i < cphr.length;i += 4) {
        var thisSlice = sjcl.bitArray.bitSlice(cphr,32*i,32*i + 128);
        padded_stuff = sjcl.bitArray.concat(padded_stuff,a.decrypt(thisSlice));
        //console.log("Zapped another block: " + a.decrypt(thisSlice));
    }
    var popped = 0;
	var popcorn = 0;
    while (popped == 0) {
		popcorn += 1;
		if (popcorn >= 512) break; //this is bad if it happens, but should minimize loops
        var lastElem = sjcl.bitArray.bitSlice(padded_stuff,sjcl.bitArray.bitLength(padded_stuff) - 1);
		console.log("Popped a " + lastElem[0]);
        popped = lastElem[0]; //since lastElem is a bit array
        padded_stuff = sjcl.bitArray.bitSlice(padded_stuff,0,sjcl.bitArray.bitLength(padded_stuff) - 2);
        console.log("padded_stuff is " + padded_stuff);
    }
    console.log("padded_stuff is " + padded_stuff + " and " + sjcl.utf8String.fromBits(padded_stuff));
    return sjcl.utf8String.fromBits(padded_stuff);
}
...
function Decrypt(cipherText, group) {
    ...
    if (cipherText.indexOf('aes:') == 0) {
        console.log("I'm going to decrypt this.")
